        ...
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="rounded-lg shadow relative" style="background-color: #374050;">
                ...
                <div class="flex items-start justify-between p-5 border-b rounded-t" style="border-color: rgba(255,255,255,0.2);">
                    <h3 id="titleId" class="text-xl lg:text-2xl font-semibold text-white">
                        Добавить продукт
                    </h3>
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="create-modal" onclick="cleanWindow()">
                ...
                <div class="flex space-x-2 items-center p-6 border-t rounded-b" style="border-color: rgba(255,255,255,0.2);">
                    <button id="sendDataButtonId" class="inline-flex items-center h-10 px-9 text-xs text-white transition-colors duration-150 bg-[#1da1f2]/90 rounded-lg focus:shadow-outline hover:bg-blue-[#1da1f2]/90" style="text-align: center; border-radius: 10px; width: 132px;" onclick="sendData('{{route('product.store')}}')">
                        <span>Добавить</span>
                    </button>
                </div>
            </div>
...
<script>
    ...
    function openEditWindow()
    {
        ...
        document.getElementById('name').value = current_product.name;
        document.getElementById('article').value = current_product.article;
        @if(!auth()->user()->canEditArticle())
            document.getElementById('article').disabled = true;
        @endif
        if(current_product.status === 'Доступен')
        {
        ...
        attribute_id = 1;
        let obj = current_product.data;
        let attribute_area = document.getElementById('AttributeAreaId');
        attribute_area.innerHTML = '';
        for (let key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                divElement.innerHTML = `
                                <div>
                                    <label for="input1" class="text-white" style="font-size: 11px">Название</label>
                                    <input type="text" id="attribute_name" name="attribute_name" class="rounded-lg bg-gray-200 mt-2 p-2" value="${key}">
                                </div>
                                <div>
                                    <label for="input2" class="text-white" style="font-size: 11px">Значение</label>
                                    <input type="text" id="attribute_item" name="attribute_item" class="rounded-lg bg-gray-200 mt-2 p-2" value="${obj[key]}">
                                </div>
                                <div class="align-middle inline-block mt-8">
                                    <svg width="24" height="24" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" onclick="deleteAttribute(${attribute_id})">
                                        <path d="M6.66663 10L6.66663 8" stroke="#C4C4C4" stroke-opacity="0.5" stroke-width="2" stroke-linecap="round"/>
        ...
        let button = document.getElementById('sendDataButtonId');
        button.innerHTML = `<span>Сохранить</span>`;
        button.onclick = null;
        button.addEventListener('click', function() {
            sendData(`/product/update/${current_product.id}`);
        });

        document.getElementById('titleId').innerText = `Редактировать ${current_product.article}`;
        document.getElementById('openShowModal').click();
        document.getElementById('openCreateModel').click();
    }


    function cleanWindow()
    {
        document.getElementById('AttributeAreaId').innerHTML = '';
        ...
        button.onclick = null;
        button.addEventListener('click', function() {
            sendData(`{{route('product.store')}}`)
        });
        document.getElementById('titleId').innerText = 'Добавить продукт';
        removeErrors();
    }
</script>
